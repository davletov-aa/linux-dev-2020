...
void process_inputs(PCRE2_SPTR pattern, PCRE2_SPTR subject, WINDOW *winO)
{
    pcre2_code *re;
    ...
    re = pcre2_compile(pattern, PCRE2_ZERO_TERMINATED, PCRE_OPTIONS, &errnum, &erroffs, NULL);
...
int main(int argc, char **argv) {
	...
	do {
		...
		wprintw(winO, " \'%s\': \'%s\'\n", inA, inB);
		process_inputs((PCRE2_SPTR) inA, (PCRE2_SPTR) inB, winO);
		box(winO, 0, 0);
		wrefresh(winO);
	} while(*inA);
